...
기존 YouTube URL 입력의 오른쪽 내부에 폭이 고정된 상태 영역을 둔다. 텍스트가 아이콘과 겹치지 않게 오른쪽 여백을 확보한다. 상태 설명은 입력 바로 아래에 표시하고 기존 곡 정보·커버 카드는 그 아래 유지한다.
...
성공 기준과 외부 계약이 우선이며 내부 클래스·함수 분할은 자유다. 아래 표는 이후 구현 작업의 경계이며 이번 PR은 문서만 작성한다.

| 슬라이스 | 산출물 | 소유(수정 가능) 경로 | 수정 금지 경로 | 선행 조건 | 상태 |
| --- | --- | --- | --- | --- | --- |
| Y1 | 공통 URL·상태·IPC 타입, preload 계약 | `src/shared/types.ts`, `src/shared/youtubeUrl.ts`, `src/shared/__tests__/youtubeUrl.test.ts`, 신규 `src/shared/__tests__/youtubePreview.test.ts`, `src/preload/**` | Y2·Y3·Y4 소유 경로 및 sidecar | 없음; Y2·Y3 시작 전 계약 확정 | 미착수 |
| Y2 | Main 조회·취소·분류·썸네일·제출 검증과 fixture | `src/main/library/YtDlpService.ts`, 신규 `src/main/library/YoutubePreviewService.ts`, `src/main/library/importUrlPrecheck.ts`, `src/main/library/__tests__/YtDlpService.test.ts`, `src/main/library/__tests__/fake_ytdlp.mjs`, `src/main/library/__tests__/importUrlPrecheck.test.ts`, 신규 `src/main/library/__tests__/YoutubePreviewService.test.ts`, `src/main/ipc.ts`, `src/main/index.ts` | Y1·Y3·Y4 소유 경로 및 sidecar | Y1 계약 확정 | 미착수 |
| Y3 | 입력 아이콘·자동 조회·폼 보존·이미지·접근성 | `src/renderer/src/components/ImportDialog.tsx`, `src/renderer/src/import/**`, `src/renderer/src/assets/import-dialog.css` | Y1·Y2·Y4 소유 경로 및 sidecar | Y1 계약 확정; Y2와 병렬 구현 가능 | 미착수 |
| Y4 | 통합 검증·문서·필요한 설정 조정 | `docs/**`, `package.json`, `pnpm-lock.yaml`, `vitest.config.ts`, 신규 `tests/youtube-preview/**` | Y1·Y2·Y3 소유 경로 및 sidecar | Y1-Y3 결과 통합 | 미착수 |
...
아래 구현 검증은 향후 구현 완료 판정용이다. 이번 문서 PR에서는 문서 구조·참조·코드 사실 일치만 검증한다.
