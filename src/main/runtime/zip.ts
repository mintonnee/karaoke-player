...
export interface ArchiveAllowSpec {
  sha256: string
  size: number
  executable?: boolean
  dest?: string
...
export function listZipEntries(input: Buffer | Uint8Array): {
  entries: ZipListEntry[]
  buf: Buffer
} {
  const buf = Buffer.isBuffer(input) ? input : Buffer.from(input)
  const eocd = findEocd(buf)
  if (eocd >= 20 && readU32(buf, eocd - 20) === ZIP64_EOCD_LOCATOR) {
...
export function readZipFileData(buf: Buffer, entry: ZipListEntry): Buffer {
...
export function extractZipVerified(
