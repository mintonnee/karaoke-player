...
- 작성일: 2026-09-02
- 후속 확장: 곡 수정 창의 명시적 재측정·초안 반영은 `011-track-reanalysis.md`를 따른다 (구현 완료, 실제 앱 수동 확인 대기). 기존 자동 분석·사용자 값 보호 계약은 유지한다.
...
| A4       | v2 알고리즘 적용: Bellman–Budge·110 Hz·접기 170·`ANALYSIS_VERSION = 2`, 키 `?` 제거, 합성 테스트·정확도 회귀 테스트 | `sidecar/src/karaoke_worker/analyze.py`, `sidecar/tests/**`, `src/shared/types.ts`(상수만), `src/shared/musicKey.ts`, `src/shared/__tests__/musicKey.test.ts`, `src/renderer/src/App.tsx`·`src/renderer/src/components/Transport.tsx`(표시 로직만), `src/main/library/__tests__/**`(상수 참조 갱신) | `src/main/library/*.ts`, `sidecar/src/karaoke_worker/cli.py`, `docs/**`, `README.md` | A1–A3 완료. 단일 슬라이스(순차)                   | 완료 |
...
- **v2 적용 (A4, 2026-09-02, 감독자 직접)**: `analyze.py`의 프로파일 상수(`PROFILE_MAJOR`/`PROFILE_MINOR`, Bellman–Budge)·`KEY_FMIN = 110`·`BPM_MAX = 170`·`ANALYSIS_VERSION = 2`, `types.ts`의 `ANALYSIS_VERSION = 2`와 `KEY_LOW_CONF` 제거, `formatKeyDisplay(key)`에서 신뢰도 인자 제거(키 `?` 없음). 합성 신호 테스트는 Bellman–Budge에서도 C/Am을 그대로 통과했다. 정답 회귀 테스트 `test_ground_truth_regression`은 `%APPDATA%`의 라이브러리에서 제목으로 13곡을 찾아 BPM 2%·키 허용 집합(상대조 포함)을 검사하며, 8곡 미만이면 skip, 키 오판은 1곡(폭망)까지 허용한다. 가짜 워커는 보고할 버전을 argv로 받아 `ANALYSIS_VERSION`을 그대로 돌려준다(하드코딩 1이 백필 테스트를 깨뜨렸다). 앱 실행으로 17곡 전부 `analysis_version=2`로 자동 재분석됐고 결과는 위 표의 v2 열과 일치한다(廻廻奇譚 93.7 F, ポルターガイスト 100 G, 千鳥 120 D#, 폭망은 여전히 D).
...
| 기준 | 상태           | 근거                                                                                                                                                   |
| ---- | -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | 자동 검증 완료 | `pytest sidecar/tests` 11건 exit 0 (v2 프로파일로 재통과. 키 순수 함수·BPM·end-to-end·시간 상한·체크포인트 실패 격리)                                  |
...
| 6    | 단위 검증      | `LibraryStore.test.ts`(정규식·범위 거부, user 보호, undefined 유지), `musicKey.test.ts`(키 `?` 없음). 폼 입력·재시작 유지는 수동 확인 대기             |
| 7    | 자동 검증 완료 | `pnpm typecheck && pnpm lint && pnpm test` exit 0 (11 files, 123 tests, v2 반영 후 재실행)                                                             |
| 8    | 수동 확인 완료 | §4.5 실측: CUDA 2.4초, CPU 5.5초 (3분 45초 곡)                                                                                                         |
| 9    | 자동+수동 완료 | `test_ground_truth_regression` 13곡 통과(BPM 13/13, 키 12/13). 앱 실행 후 17곡 `analysis_version=2`, 사용자 값 없음이라 보존 검증은 단위 테스트로 대체 |
